    ...
    uploadProfileImage: async (req, res) => {
        try {
            ...
            if(file.mimetype !== 'image/png' && file.mimetype !== 'image/jpeg') {
                removeTemp(file.tempFilePath)
                return res.status(StatusCodes.BAD_REQUEST).json({ msg: "Please upload .jpg or .png image format" })
            }

            // upload logic
            await cloudinary.v2.uploader.upload(file.tempFilePath, { folder: 'profile_image' }, (err, result) => {
                if(err) return res.status(StatusCodes.BAD_REQUEST).json({ msg: err.message })
                removeTemp(file.tempFilePath)
